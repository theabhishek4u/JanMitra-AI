...
const initialComplaints = [
  ...
  },
  {
    ...
    priority: 'medium',
    status: 'resolved',
    department_id: 'lmc', // Corrected from 'lesa' to match valid department ID 'lmc'
    latitude: 26.8400,
    longitude: 80.9600,
    ...
    trust_score: 0.98,
    assigned_officer: 'Smt. Neha Tripathi' // Corrected from 'Smt. Meena Devi' to match department officer Smt. Neha Tripathi
  },
  {
    id: 'JM-2026-003-777',
    ...
    priority: 'high',
    status: 'submitted',
    department_id: 'jal-nigam', // Corrected from 'jal-sansthan' to match valid department ID 'jal-nigam'
    latitude: 26.8600,
    longitude: 80.9400,
    ...
    trust_score: 0.99,
    assigned_officer: 'Smt. Priya Sharma' // Corrected from 'Shri Anil Dubey' to match department officer Smt. Priya Sharma
  },
  {
    id: 'JM-2026-004-666',
    ...
    category: 'Road Damage', // Match with correct category name
    ...
    trust_score: 0.96,
    assigned_officer: 'Shri Amit Verma' // Corrected from 'Shri R. K. Singh' to match department officer Shri Amit Verma
  },
  {
    id: 'JM-2026-005-555',
    ...
    category: 'Public Health', // Remapped to Public Health since Animal Control is not a standard matrix tab
    ...
    trust_score: 0.85,
    assigned_officer: 'Dr. Sanjay Pandey' // Corrected to CMO Dr. Sanjay Pandey
  },
  {
    id: 'JM-2026-006-444',
    ...
    category: 'Garbage / Sanitation', // Remapped to Sanitation
    category_hi: 'कचरा / स्वच्छता',
    priority: 'high',
    status: 'resolved',
    department_id: 'nagar-nigam',
  ...
  },
  {
    ...
    priority: 'low',
    status: 'department_assigned',
    department_id: 'municipal-authority', // Corrected from nagar-nigam
    latitude: 26.8350,
    longitude: 80.9550,
    ...
    trust_score: 0.92,
    assigned_officer: 'Shri Deepak Gupta' // Corrected from Smt. Sunita Yadav
  },
  {
    id: 'JM-2026-008-222',
  ...
  },
  {
    ...
    category: 'Garbage / Sanitation', // Remapped from Parks
    category_hi: 'कचरा / स्वच्छता',
    priority: 'low',
    status: 'resolved',
    department_id: 'nagar-nigam',
    ...
    trust_score: 0.80,
    assigned_officer: 'Shri Rajesh Kumar' // Corrected to Shri Rajesh Kumar
  },
  {
    id: 'JM-2026-010-000',
    ...
    priority: 'high',
    status: 'department_assigned',
    department_id: 'power-dept', // Corrected from lesa
    latitude: 26.8650,
    longitude: 80.9450,
    area: 'Chowk, Lucknow',
    citizen_id: 'user-10',
    ...
    trust_score: 1.00,
    assigned_officer: 'Shri Vikram Singh' // Corrected from Smt. Meena Devi
  }
];
...
function generateSeedSql() {
  ...
  for (let i = 0; i < rawComplaints.length; i++) {
    ...
    const indexStr = String(11 + i).padStart(3, "0"); // Start from 011
    const uniqueId = `JM-2026-${indexStr}-${100 + (i * 17) % 900}`;
    const dept = getDeptDetails(raw.category);
    const coords = areaCoords[raw.area] || { lat: 26.8467, lng: 80.9462 };
    const status = statuses[i % statuses.length];
    const aiConfidence = parseFloat((0.80 + (i * 0.013) % 0.19).toFixed(2));
    
    let trustLevel = "high";
    let spamProbability = parseFloat(((i * 0.003) % 0.05).toFixed(2));
    
    if (raw.category === "Corruption" || raw.category === "Illegal Construction") {
      if (i % 3 === 0) {
        trustLevel = "low";
        spamProbability = parseFloat((0.75 + (i * 0.01) % 0.2).toFixed(2));
      } else if (i % 3 === 1) {
        trustLevel = "medium";
        spamProbability = parseFloat((0.35 + (i * 0.01) % 0.25).toFixed(2));
      }
    }
    ...
    allComplaints.push({
      ...
      latitude: parseFloat((coords.lat + ((i * 0.0003) % 0.004) - 0.002).toFixed(6)),
      longitude: parseFloat((coords.lng + ((i * 0.0003) % 0.004) - 0.002).toFixed(6)),
      area: raw.area,
      ...
      citizen_phone: `+91 9${Math.floor(812345678 + (i * 1234567) % 180000000)}`,
  ...
  // Format complaints sql list
  const complaintRows = allComplaints.map((c, i) => {
    return `  (
    ${esc(c.id)}, 
    ${esc(c.title)}, 
    ${esc(c.title_hi)}, 
    ${esc(c.description)}, 
    ${esc(c.description_hi)}, 
    ${esc(c.category)}, 
    ${esc(c.category_hi)}, 
    ${esc(c.priority)}, 
    ${esc(c.status)}, 
    ...
    ${esc(c.ai_summary_hi)}, 
    ${c.ai_confidence}, 
    ${jsonEsc(c.trust_analysis)}, 
    ${c.trust_score}, 
    ${esc(c.assigned_officer)}
  )`;
  });

  sql += complaintRows.join(",\n") + ";\n\n-- Seed timeline updates\nINSERT INTO public.complaint_updates (complaint_id, status, message, message_hi)\nVALUES\n";

  // Timeline updates sql list
  const updateRows = [];
  allComplaints.forEach((c) => {
    const dept = getDeptDetails(c.category);
    updateRows.push(`  (${esc(c.id)}, 'submitted', 'Complaint submitted successfully by citizen.', 'नागरिक द्वारा शिकायत सफलतापूर्वक दर्ज की गई।')`);
    
    if (c.status !== "submitted") {
      updateRows.push(`  (${esc(c.id)}, 'department_assigned', 'Routed to department automatically.', 'स्वचालित रूप से आवंटित।')`);
    }
    
    if (c.status === "officer_reviewing" || c.status === "resolved") {
      updateRows.push(`  (${esc(c.id)}, 'officer_reviewing', 'Officer ${dept.officer} has begun reviewing the grievance.', 'अधिकारी ${dept.officer} ने शिकायत की समीक्षा शुरू कर दी है।')`);
    }
    
    if (c.status === "resolved") {
      updateRows.push(`  (${esc(c.id)}, 'resolved', 'Issue resolved completely. Resolution verified by department.', 'समस्या का पूर्ण निवारण हो चुका है। विभाग द्वारा निवारण की पुष्टि की गई है।')`);
    }
  });
  ...
  console.log("Successfully generated supabase/seed.sql with 55 complaints!");

  // Now let's generate the setup-supabase.js file contents as well
  let jsFile = `const { createClient } = require('@supabase/supabase-js');
const fs = require('fs');
...
async function seedData() {
  ...
  // Insert complaints
  const complaints = ${JSON.stringify(allComplaints, null, 2)};
  ...
  allComplaints.forEach((c) => {
    const dept = getDeptDetails(c.category);
    jsUpdates.push(`    { complaint_id: '${c.id}', status: 'submitted', message: 'Complaint submitted successfully by citizen.', message_hi: 'नागरिक द्वारा शिकायत सफलतापूर्वक दर्ज की गई।' }`);
    
    if (c.status !== "submitted") {
      jsUpdates.push(`    { complaint_id: '${c.id}', status: 'department_assigned', message: 'Routed to department automatically.', message_hi: 'स्वचालित रूप से आवंटित।' }`);
    }
    
    if (c.status === "officer_reviewing" || c.status === "resolved") {
      jsUpdates.push(`    { complaint_id: '${c.id}', status: 'officer_reviewing', message: 'Officer ${dept.officer} has begun reviewing the grievance.', message_hi: 'अधिकारी ${dept.officer} ने शिकायत की समीक्षा शुरू कर दी है।' }`);
    }
    
    if (c.status === "resolved") {
      jsUpdates.push(`    { complaint_id: '${c.id}', status: 'resolved', message: 'Issue resolved completely. Resolution verified by department.', message_hi: 'समस्या का पूर्ण निवारण हो चुका है। विभाग द्वारा निवारण की पुष्टि की गई है।' }`);
    }
  });

  jsFile += jsUpdates.join(",\n") + `\n  ];
...
`;
  ...
  fs.writeFileSync('setup-supabase.js', jsFile, 'utf8');
  console.log("Successfully generated setup-supabase.js with 55 complaints!");
}
